...
function function_creator(line) {
    ...
    if((/^[A-Za-z]([A-Za-z0-9_])*$/).test(line[1])){
            ...
            for(pos=3;i<line.length;pos++){
                ...
                if((/^[A-Za-z]([A-Za-z0-9_])*$/).test(line[pos])){
                    ...
                    if(line[pos+1]==")"){
                        ...
                        throw new SyntaxError("Invalid argument line")
                    }
                }
                else{
                    throw new SyntaxError("Expecting argument after function parentheses opening")
                }
                
            }
            if(line[i]!=")"){
                throw new SyntaxError("Expect closing parentheses after function argument")
            }
            else{
                if(line[++pos]=="{"){
        ...
        throw SyntaxError("Expecting something after function declaration")
    }
...
function Main(){
    ...
    function read_line(this_line){
       ...
       if(line_array){
        ...
        if(line_array[0]=="function"){
                ...
                return expression_handling(line_array,current_pos[0])
            }
        }
        else{
   ...
   read_line(line1);
   let line2 = "10*50+30*20-10";
   output.push(read_line(line2));
   let line3 =  "((90-10)+(+(100+30*20+10*-30+0-40)+3%2*3^4)*5)+&20*4"
   output.push(read_line(line3));
   let line4 = "tien(5,2,((90-10)+(+(100+30*20+10*-30+0-40)+3%2*3^4)*5)+&20*4)";
   output.push(read_line(line4));
   return output;
}
console.log(Main());
